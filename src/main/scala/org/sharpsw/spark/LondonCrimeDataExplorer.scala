...
object LondonCrimeDataExplorer {
  ...
  private def row(line: List[String]): Row = {
    val list =
      List(line.head, line(1), line(2), line(3), line(4).toInt, line(5).toInt, line(6).toInt)
    Row.fromSeq(list)
  }
  ...
  def extractCombinedCategories(contents: DataFrame): DataFrame =
    contents
      ...
      .distinct
      .sort("major_category", "minor_category")

  def calculateTotalCrimeCountByBorough(contents: DataFrame): DataFrame =
    contents
      .groupBy(contents("borough"))
      .agg(sum(contents("value")).alias("total_crimes"))
      .sort(desc("total_crimes"))

  def calculateCrimesByMajorCategory(contents: DataFrame): DataFrame =
    contents
      .groupBy(contents("major_category"))
      .agg(sum(contents("value")).alias("total"))
      .sort(desc("total"))

  def calculateCrimeCountByMinorCategory(contents: DataFrame): DataFrame =
    contents
      .groupBy(contents("major_category"), contents("minor_category"))
      .agg(sum(contents("value")).alias("total"))
      .sort(desc("total"))

  def calculateCrimeCountByBoroughAndYear(contents: DataFrame): DataFrame =
    contents
      .groupBy(contents("borough"), contents("year"))
      .agg(sum(contents("value")).alias("total"))
      .sort(asc("year"), asc("borough"))

  def calculateCrimesByMajorCategoryAndYear(contents: DataFrame): DataFrame =
    contents
      .groupBy(contents("major_category"), contents("year"))
      .agg(sum(contents("value")).alias("total"))
      .sort(desc("year"), desc("total"))

  def calculateCrimesByMinorCategoryAndYear(contents: DataFrame): DataFrame =
    contents
      .groupBy(contents("major_category"), contents("minor_category"), contents("year"))
      .agg(sum(contents("value")).alias("total"))
      .sort(desc("year"), desc("total"))
  ...
  def calculateCrimesByYearAndMonth(contents: DataFrame): DataFrame =
    contents
      .groupBy(contents("year"), contents("month"))
      .agg(sum(contents("value")).alias("total"))
      .sort(desc("year"), desc("month"))

  def calculateCrimePercentageByCategoryByYear(
    contents: DataFrame,
    year: Int,
    ...
    filtered
      .groupBy(filtered("major_category"))
      .agg(sum(filtered("value")).alias("occurrences"))
      .sort(desc("occurrences"))
      .withColumn("total", sum(col("occurrences")).over(byMajorCategory))
      .withColumn("percentage", col("occurrences") * 100 / col("total"))
      .drop(col("occurrences"))
      .drop(col("total"))
  }
  ...
  def calculateCrimesPercentageByCategoryAndYear(
    contents: DataFrame,
    sparkSession: SparkSession): List[(Int, DataFrame)] =
    getYearList(contents, sparkSession).map(item =>
      (item, calculateCrimePercentageByCategoryByYear(contents, item, sparkSession)))

  def calculateTotalCrimesByYearLsoaCode(contents: DataFrame): DataFrame =
    contents
      .groupBy(contents("year"), contents("borough"), contents("lsoa_code"))
      .agg(sum(contents("value")).alias("total"))
      .sort(desc("year"), asc("borough"), asc("lsoa_code"))
}
